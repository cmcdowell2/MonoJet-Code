...
double MonoJets::getSF(int lepindex_leading, int lepindex_subleading) {
  ...
  double looseMuISO_SF_corr = h_looseMuSF_ISO->GetBinContent(h_looseMuSF_ISO->GetXaxis()->FindBin(subleadingMuPt),h_looseMuSF_ISO->GetYaxis()->FindBin(fabs(subleadingMuEta)));
  double looseMuID_SF_corr = h_looseMuSF_ID->GetBinContent(h_looseMuSF_ID->GetXaxis()->FindBin(subleadingMuPt),h_looseMuSF_ID->GetYaxis()->FindBin(fabs(subleadingMuEta)));
...
bool MonoJets::btagVeto() {
  bool btagVeto = true;
  for (int i = 0; i < nJet; i++) {
    if (jetPt->at(i) >20.0 && fabs(jetEta->at(i)) < 2.5 && jetCSV2BJetTags->at(i) > 0.84)
      btagVeto = false;
  }
  return btagVeto;
}
...
vector<int> MonoJets::electron_veto_looseID(int jet_index, int leading_mu_index, int subleading_mu_index, float elePtCut) {
  ...
  for (int i = 0; i < nEle; i++) {
	...
	if (elePt->at(i) > elePtCut) {
	  ...
	  if (dR_subleading > 0.3 && dR_leading > 0.3 && deltaR(eleEta->at(i),elePhi->at(i),jetEta->at(jet_index),jetPhi->at(jet_index)) > 0.5) {
            ele_cands.push_back(i);
          }
	}
      }
