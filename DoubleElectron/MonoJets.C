...
/*bool MonoJets::getJetHEMVeto(double jetPtCut) {
  bool pass = true;
...
double MonoJets::getSF(int lepindex_leading, int lepindex_subleading) {
...
void MonoJets::Loop(Long64_t maxEvents, int reportEvery) {
  ...
  double nTotalEvents,nFilters, nHLT, nCRSelection, nMET200, ndilepton, nNoMuons, nMETcut,nbtagVeto, nDphiJetMET,nJetSelection, nEleHEM;
  nTotalEvents = nFilters = nHLT = nCRSelection = nMET200 = ndilepton = nNoMuons = nMETcut = nDphiJetMET = nbtagVeto = nJetSelection = nEleHEM = 0;
	  ...
	  vector<int> elelist_leading = electron_veto_tightID(jetCand[0],40.0);
          ...
          if (elelist.size() == 2) {
            ...
            if (elePairSet) {
              ...
              if (!sample.isData) 
                event_weight *= getSF(lepindex_leading, lepindex_subleading);
...
void MonoJets::BookHistos(const char* file2) {
  ...
  h_cutflow->GetXaxis()->SetBinLabel(8,"nNoMuons");
  ...
  //h_cutflow->GetXaxis()->SetBinLabel(13,"JetHEMVeto");  
  ...
  for(int i=0; i<nHisto; i++) {
    ...
    h_leadingLeptonEtaPhiComp[i] = new TH2F(("h_leadingLeptonEtaPhiComp"+histname).c_str(), "h_leadingLeptonEtaPhiComp", 50, -3.0, 3.0, 50, -3.1416, 3.1416);
    h_subleadingLeptonEtaPhiComp[i] = new TH2F(("h_subleadingLeptonEtaPhiComp"+histname).c_str(), "h_subleadingLeptonEtaPhiComp", 50, -3.0, 3.0, 50, -3.1416, 3.1416);
...
void MonoJets::fillHistos(int histoNumber,double event_weight) {
  ...
  if(lepindex_leading >= 0 && lepindex_subleading >= 0) { 
    ...
    h_leadingLeptonEtaPhiComp[histoNumber]->Fill(eleEta->at(lepindex_leading),elePhi->at(lepindex_leading),event_weight);
    ...
    h_subleadingLeptonEtaPhiComp[histoNumber]->Fill(eleEta->at(lepindex_subleading),elePhi->at(lepindex_subleading),event_weight);
  }
...
vector<int> MonoJets::JetVetoDecision(int leading_lep_index, int subleading_lep_index) {
  bool jetVeto=true;
  vector<int> jetindex;
...
bool MonoJets::btagVeto() {
  bool btagVeto = true;
  for(int i = 0; i < nJet; i++) {
    if (jetPt->at(i) >20.0 && jetEta->at(i) < 2.4 && jetCSV2BJetTags->at(i) > 0.84)
      btagVeto = false;
  }
  return btagVeto;
}
...
vector<int> MonoJets::electron_veto_looseID(int jet_index, int leading_mu_index, int subleading_mu_index, float elePtCut) {
  vector<int> ele_cands;
  ele_cands.clear();
...
vector<int> MonoJets::muon_veto_looseID(int jet_index, int leading_ele_index, int subleading_ele_index, float muPtCut) {
  vector<int> mu_cands;
  mu_cands.clear();
  ...
  for (int i = 0; i < nMu; i++) {
    if (muIDbit->at(i)>>0&1==1) {
	  ...
	  if (dR_leadingEle > 0.4 && dR_subleadingEle > 0.4 && deltaR(muEta->at(i),muPhi->at(i),jetEta->at(jet_index),jetPhi->at(jet_index)) > 0.4) {
            mu_cands.push_back(i);
          }
	}
      }
